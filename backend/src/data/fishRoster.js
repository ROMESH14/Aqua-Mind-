function folderToName(folder, aliases) {
  if (aliases[folder]) return aliases[folder];
  return String(folder || '').replace(/_s_/g, "'s ").replace(/_/g, ' ').replace(/\s+/g, ' ').trim();
}
...
const MONSTER_FOLDERS = [
  'Asian_Arowana_Golden_Red_Dragon_Fish', 'Black_Arowana', 'Black_Ghost_Knifefish', 'Blood_Parrot_Cichlid',
  'Clown_Knifefish', 'Clown_Loach', 'Common_Goldfish', 'Convict_Cichlid', 'Electric_Blue_Acara',
  'Elephant_Nose_Fish', 'Fire_Eel', 'Firemouth_Cichlid', 'Flowerhorn_Cichlid', 'Freshwater_Angelshark_Bala_Shark',
  'Frontosa_Cichlid', 'Giant_Gourami', 'Green_Terror', 'Indonesian_Tiger_Fish_Datnoid', 'Iridescent_Shark_Pangasius',
  'Jack_Dempsey', 'Jaguar_Cichlid', 'Jardini_Arowana_Australian_Arowana', 'Jewel_Cichlid', 'Kissing_Gourami',
  'Motoro_Freshwater_Stingray', 'Oscar', 'Peacock_Cichlid', 'Pictus_Catfish', 'Rainbow_Cichlid', 'Rainbow_Shark',
  'Red-bellied_Pacu', 'Redtail_Catfish', 'Red_Tail_Shark', 'Ropefish', 'Senegal_Bichir_Dinosaur_Eel',
  'Silver_Arowana', 'Tiger_Shovelnose_Catfish', 'Tinfoil_Barb',
];

const SHRIMP_FOLDERS = [
  'Amano_Shrimp', 'Bamboo_Shrimp_Wood_Shrimp', 'Blue_Dream_Shrimp', 'Cardinal_Sulawesi_Shrimp',
  'Cherry_Shrimp', 'Red_Rili_Shrimp',
];

const SMALL_FOLDERS = [
  'African_Butterflyfish', 'Angelfish', 'Apistogramma_Cockatoo_Dwarf_Cichlid', 'Betta_Siamese_Fighting_Fish',
  'Black_Neon_Tetra', 'Bleeding_Heart_Tetra', 'Bloodfin_Tetra', 'Blue_Gourami', 'Boesemani_Rainbowfish',
  'Bolivian_Ram', 'Bristlenose_Pleco', 'Bronze_Corydoras', 'Buenos_Aires_Tetra', 'Cape_Lopez_Killifish',
  'Cardinal_Tetra', 'Celebes_Rainbowfish', 'Celestial_Pearl_Danio_Galaxy_Rasbora', 'Checkerboard_Barb',
  'Cherry_Barb', 'Chili_Rasbora', 'Chocolate_Gourami', 'Clown_Pleco', 'Congo_Tetra', 'Diamond_Tetra',
  'Discus', 'Dwarf_Chain_Loach', 'Dwarf_Gourami', 'Electric_Yellow_Cichlid', 'Ember_Tetra',
  'Endler_s_Livebearer', 'Fancy_Goldfish_Oranda', 'Farlowella_Catfish_Twig_Catfish', 'Flying_Fox',
  'Freshwater_Pufferfish_Dwarf_Puffer', 'German_Blue_Ram', 'Giant_Danio', 'Glass_Catfish', 'Glowlight_Tetra',
  'Gold_Barb', 'Green_Neon_Tetra', 'Guppy', 'Harlequin_Rasbora', 'Hillstream_Loach_Borneo_Sucker',
  'Honey_Gourami', 'Keyhole_Cichlid', 'Kribensis', 'Kuhli_Loach', 'Lemon_Tetra', 'Licorice_Gourami',
  'Molly', 'Neon_Dwarf_Rainbowfish', 'Neon_Tetra', 'Odessa_Barb', 'Otocinclus_Catfish', 'Panda_Corydoras',
  'Paradise_Fish', 'Peacock_Gudgeon', 'Pearlscale_Geophagus', 'Pearl_Danio', 'Pearl_Gourami', 'Penguin_Tetra',
  'Peppered_Corydoras', 'Platy', 'Roseline_Shark', 'Rosy_Barb', 'Rummy_Nose_Tetra', 'Serpae_Tetra',
  'Silver_Tip_Tetra', 'Sterbai_Corydoras', 'Swordtail', 'Threadfin_Rainbowfish', 'Tiger_Barb',
  'Turquoise_Rainbowfish', 'Weather_Loach_Dojo_Loach', 'White_Cloud_Mountain_Minnow', 'Yoyo_Loach',
  'Zebra_Danio', 'Zebra_Loach',
];

const MONSTER_FISH = [...MONSTER_FOLDERS.map((folder) => folderToName(folder, NAME_ALIASES)), 'Silver Dollar'];
const SHRIMP = SHRIMP_FOLDERS.map((folder) => folderToName(folder, NAME_ALIASES));
const SMALL_AND_SHRIMP = [
  ...SMALL_FOLDERS.map((folder) => folderToName(folder, NAME_ALIASES)),
  ...SHRIMP,
];

function isShrimpName(name) {
  return /shrimp|prawn/i.test(String(name || ''));
}

function pickTopStocking(scored = [], volumeLiters, limit = 4) {
...
function plantVsFishDelta(plantName, fishNames = []) {
...
function plantCompatNote(plantName, fishNames = []) {
